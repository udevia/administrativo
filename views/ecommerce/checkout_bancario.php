...
<div class="bg-white rounded-2xl border border-slate-200 p-5 shadow-sm space-y-4" x-data="checkoutBancarioApp()">
   <div class="flex items-center justify-between border-b pb-3">
       ...
       <span class="text-[11px] font-mono font-bold text-emerald-700 bg-emerald-50 px-2.5 py-0.5 rounded-full border bo
   ...
   <div class="grid grid-cols-1 sm:grid-cols-2 gap-3 text-xs">
       ...
       <div>
           ...
           <div class="flex gap-1">
               ...
               <input type="text" x-model="pago.numero_cedula" placeholder="12345678" class="w-full border rounded-xl p
           </div>
       </div>
       <div>
           <label class="block font-semibold text-slate-600 mb-1">Teléfono Afiliado a Pago Móvil:</label>
           <input type="text" x-model="pago.telefono" placeholder="04141234567" class="w-full border rounded-xl p-2.5 b
       </div>
       <div>
           <label class="block font-semibold text-slate-600 mb-1">Clave de Pago / Token OTP (C2P):</label>
           <input type="password" maxlength="8" x-model="pago.token_otp" placeholder="Solicítela en su App bancaria" cl
       </div>
   </div>
   ...
   <button @click="ejecutarPagoC2P()" :disabled="procesando || !pago.telefono || !pago.numero_cedula || !pago.token_otp
       ...
       async ejecutarPagoC2P() {
           ...
           try {
               ...
               if (data.status === 'success') {
                   alert(`¡Pago Aprobado! Referencia Bancaria: ${data.referencia_bancaria}. Su compra ha sido confirmad
